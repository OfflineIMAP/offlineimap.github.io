...
If you don't get hints here, feel free to ask in our [mailing list]({{
links.mailing_list.address }}) or search in [the archives]({{
...
an mbox-backend, though if someone sent me well-written mbox support and pledged
...
IMAP servers use a folders UIDVALIDITY value in combination with a unique ID
...
{:.note}
If you're using the SQLite backend, change the above `LocalStatus` to
`LocalStatus-sqlite`.
 
...
 
Do move the directory from the metadata path (`~/Mail` in this sample). **There
should be NO MORE `INBOX` directory in it.**
 
The reason is that moving it like this means like *"I've never synced this
folder before, download it on next sync"*. Only deleting the **content** (the
mails) means *"I've locally removed the mails, remove them on the server on
next sync"*.
...
This question comes up frequently on the [mailing list archive]({{
...
We don't have well testing tool, for now. As a IMAP client, we need an available
